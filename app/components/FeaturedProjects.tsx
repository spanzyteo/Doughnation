      ...
      <div className="overflow-hidden w-[90%] flex mt-16 mx-auto xl:w-[1200px]">
        ...
        >
          {projects.map((item) => (
            <div
              key={item.id}
              className="w-full md:min-w-[calc(100%/3.1)] min-w-full aspect-[4/3] relative"
            >
              <Image
                src={item.image}
                alt={`Project ${item.id}`}
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#1b1f2e] to-transparent"></div>
              <div className="absolute top-24 left-6 flex flex-col w-[90%]">
                <div className="flex items-center">
                  <button className="bg-[#674DF0] text-white px-4 py-1 uppercase text-[11px] cursor-pointer font-thin">
                    {item.category}
                  </button>
                  <div className="flex items-center">
                    <AiOutlineClockCircle className="text-white h-[20px] w-[20px] ml-6" />
                    <h1 className="text-white ml-1 text-[13px]">
                      {item.days} Days left
                    </h1>
                  </div>
                </div>
                <h1 className="text-white font-bold text-2xl mt-6 w-[270px]">
                  {item.title}
                </h1>
                ...
                <div className="relative w-full h-[7px] bg-white mt-3">
                  <div
                    className="absolute top-0 left-0 h-full bg-[#674DF0]"
                    style={{
                      width: `${(item.fundsRaised / item.fundsToRaise) * 100}%`,
                    }}
                  ></div>
                </div>
              </div>
